...
export const placementsController = {
  ...
  async bulkPushToPool(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      // Course Completion certificate requests start their approval clock
      // the moment a student actually lands in the pool.
      await Promise.all([...needsClockStart, ...alreadyClocked].map((id) => ensureCourseCompletionCertRequest(id)));
  ...
  async getStudentProfile(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      // Rank card per enrollment — shared with the certificate approval
      // review screen (see certificateRequests.controller.ts), which needs
      // this same breakdown alongside the student's profile.
      const rankCards = await computeRankCard(id);
